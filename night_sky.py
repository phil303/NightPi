...
def in_ra_range(ra, lng, plus_minus):
    angle_diff = abs(lng - ra) % 360
    return angle_diff < plus_minus or 360 - angle_diff < plus_minus

def in_dec_range(dec, lat, plus_minus):
    return abs(lat - dec) < plus_minus
...
def current_night_sky(lat, lng):
    ...
    with open(VISIBLE_STAR_PATH) as f:
        ...
        for row in reader:
            ...
            if in_ra_range(star_ra, lmst, 90) and in_dec_range(star_dec, lat, 90):
